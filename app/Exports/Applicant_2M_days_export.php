...
class Applicant_2M_days_export implements FromCollection, WithHeadings
{
    protected $end_date;
...
//dd($this->end_date,$this->start_date);
        $result1= Applicant::select(
            'applicants.id','applicants.applicant_phone', 'applicants.applicant_name','applicants.applicant_homePhone','applicants.applicant_job_title',
            'applicants.applicant_postcode','applicants.is_CV_reject','applicants.applicant_source','applicants.applicant_notes')
            ->where(function($query){
            $query->doesnthave('CVNote');
        })
            ->whereDate('applicants.updated_at', '<=', $this->start_date);
...
//            ->where("job_category", "=", $this->job_category)
        ...
        $rejects= Applicant::select(
            'applicants.id','applicants.applicant_phone', 'applicants.applicant_name','applicants.applicant_homePhone','applicants.applicant_job_title',
            'applicants.applicant_postcode','applicants.is_CV_reject','applicants.applicant_source','applicants.applicant_notes')->with('CRMNote')->with('CVNote')
            ->where(function($query){
                $query->whereHas('CVNote')
                    ->orWhereHas('CRMNote');
            })
            ->whereDate('applicants.updated_at', '<=', $this->end_date);
//            ->whereBetween('updated_at', [$this->start_date, $this->end_date]);
        if ($this->job_category == "44"){
