    ...
    */
    ...
    public function collection()
    {
			...
			$not_sents= Applicant::select(
                ...
                'applicant_postcode','applicant_source')->where(function($query){
            $query->doesnthave('CVNote');
        })->whereBetween('created_at', [$this->start_date, $this->end_date])->where("job_category", "=", $this->job_category)
    ...
    //    DB::enableQueryLog();
        $rejecteds= Applicant::select(
            'id','applicant_phone', 'applicant_name','applicant_homePhone','applicant_job_title',
            'applicant_postcode','applicant_source')->with(['CRMNote.History'=>function($query){
                $query->whereIn('sub_stage', ["crm_dispute","crm_interview_not_attended","crm_declined","crm_request_reject","crm_reject","crm_start_date_hold", "crm_start_date_hold_save"]);
            }])->with(['CVNote.History'=>function($query){
                $query->where('sub_stage','quality_reject')->where('status','active');
            }])
            
        ->where(function($query){ 
            
     $query->whereHas('CVNote.History',function($query){
                $query->where('sub_stage','quality_reject')->where('status','active');
    ...
    $rejecteds->map(function($row){
        //  if(!empty($row->CVNote->History))
        //  dd($row->CVNote->History);

        $rejected_status = 'Rejected CV';;
        if(isset($row->CRMNote->sub_stage) && $row->CRMNote->History->sub_stage=='crm_dispute')
        $rejected_status = 'crm_dispute';
        else if(isset($row->CRMNote->History->sub_stage) && $row->CRMNote->History->sub_stage=='crm_interview_not_attended')
        $rejected_status = 'crm_interview_not_attended';
        else if(isset($row->CRMNote->History->sub_stage) && $row->CRMNote->History->sub_stage=='crm_declined')
        $rejected_status = 'crm_declined';
        else if(isset($row->CRMNote->History->sub_stage) && $row->CRMNote->History->sub_stage=='crm_request_reject')
        $rejected_status = 'crm_request_reject';
        else if(isset($row->CRMNote->History->sub_stage) && $row->CRMNote->History->sub_stage=='crm_reject')
        $rejected_status = 'crm_reject';
        else if(isset($row->CRMNote->History->sub_stage) && $row->CRMNote->History->sub_stage=='crm_start_date_hold')
        $rejected_status = 'crm_start_date_hold';
        else if(isset($row->CRMNote->History->sub_stage) && $row->CRMNote->History->sub_stage=='crm_start_date_hold_save')
        $rejected_status = 'crm_start_date_hold_save';
        else if(isset($row->CVNote->History->sub_stage) && $row->CVNote->History->sub_stage=='quality_cvs' && $row->CVNote->History->status=='active')
		...
		//$rejects =$clean_data->toArray();
                ...
                //print_r($rejecteds);exit();
                 $arr = array();
                    $reslut = array();
                    foreach ($rejecteds->toArray() as $key => $filter_val) {
						//echo $rejecteds[$key]['id'];exit();
