<div class="content-wrapper">
    <div class="content">
        <table class="table datatable-sorting">
            ...
            <tbody>
            @foreach($not_attended as $applicant)
                <tr>
                    ...
                    <td><a href="#" data-controls-modal="#job_details{{ $applicant->id }}-{{$applicant->sale_id}}"
                           ...
                           <td>{{ $applicant->office_name }}</td>
                    <td>{{ $applicant->unit_name }}</td>
                    <td>{{ $applicant->postcode }}</td>
                    <td>
                        <p><b>DATE:</b> {{ $applicant->crm_added_date}}</p>
                        <p><b>TIME</b> {{ $applicant->crm_added_time }}</p>
                        <p><b>NOTE:</b> {{ $applicant->details }}</p>
                    </td>
                    <td>
                        <div class="list-icons">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a href="#" data-controls-modal="#manager_details{{ $applicant->id }}-{{ $applicant->sale_id }}"
                                       data-backdrop="static"
                                       data-keyboard="false" data-toggle="modal"
                                       data-target="#manager_details{{ $applicant->id }}-{{ $applicant->sale_id }}"
                                       class="dropdown-item"><i class="icon-file-confirm"></i> Manager Details</a>
                                    <a href="#" class="dropdown-item"
                                       data-controls-modal="#revert_to_attended{{ $applicant->id }}-{{ $applicant->sale_id }}" data-backdrop="static"
                                       data-keyboard="false" data-toggle="modal"
                                       data-target="#revert_to_attended{{ $applicant->id }}-{{ $applicant->sale_id }}">
                                        <i class="icon-file-confirm"></i> Revert To Attended</a>
                ...
                <div id="revert_to_attended{{ $applicant->id }}-{{ $applicant->sale_id }}" class="modal fade" tabindex="-1">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Not Attended Notes</h5>
                            ...
                            <form action="{{ route('processCv') }}"
                                ...
                                <div class="modal-footer">
                                    <button type="submit" name="back_to_attended" value="back_to_attended"
                                            class="btn bg-orange-800 legitRipple">Attended
                                    </button>
                                </div>
                            </form>
                        </div>
